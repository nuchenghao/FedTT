...
class myFed(FedAvgTrainer):
    ...
    def loss_dynamic_batch_global_loss(self):
        self.trainloader.sampler.set_index(self.current_client.train_set_index)  # 这里还是遍历local_epoch次
        gpu_utilization = []
        train_thread = threading.Thread(target=self.train, args=())
        train_thread.start()
        ...
        loss_global = torch.zeros(self.current_client.train_set_len, device=self.device, dtype=torch.float)
        for epoch in range(self.local_epoch):
            print(f"batch size : {self.trainloader.batch_sampler.batch_size}")
            cnt = 0
            total_correct = 0
            ...
            inputs_raw, targets_raw = next(itertrainloader)
            with torch.cuda.stream(self.inference_stream):
                self.inputs[cnt], self.targets[cnt] = inputs_raw.to(self.device, non_blocking=True), targets_raw.to(
                ...
                with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
                    with torch.no_grad():
                        outputs = self.inference_net(self.inputs[cnt])
                        loss_global[total_correct:total_correct + len(targets_raw)] = self.criterion(outputs,
                                                                                                     self.targets[cnt])
                        well_classified = loss_global[
                                          total_correct:total_correct + len(targets_raw)] < global_loss_threshold
                        mis_classified = ~well_classified
                        num_well_classified = well_classified.sum()
                        num_mis_classified = mis_classified.sum()
                        num_select_well = torch.ceil(num_well_classified * self.r).int()  # 这里要注意
                        ...
                        select_well_classified_inputs = self.inputs[cnt][well_classified][:num_select_well]
                        select_well_classified_targets = self.targets[cnt][well_classified][:num_select_well]
                        self.weights[cnt] = torch.cat(
                            (torch.ones(num_mis_classified, dtype=torch.float32, device=self.device),
                             torch.full((num_select_well,), 1 / self.r, device=self.device)))
                        self.inputs[cnt] = torch.cat((self.inputs[cnt][mis_classified], select_well_classified_inputs),dim=0)
                        self.targets[cnt] = torch.cat((self.targets[cnt][mis_classified], select_well_classified_targets), dim=0)
                        self.inference_event.record()
                    ...
                    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
                        with torch.no_grad():
                            ...
                            self.weights[cnt] = torch.cat(
                                (torch.ones(num_mis_classified, dtype=torch.float32, device=self.device),
                                 torch.full((num_select_well,), 1 / self.r, device=self.device)))
                            self.inputs[cnt] = torch.cat(
                                (self.inputs[cnt][mis_classified], select_well_classified_inputs), dim=0)
                            self.targets[cnt] = torch.cat(
                                (self.targets[cnt][mis_classified], select_well_classified_targets),dim=0)
                    self.inference_event.record()
                    self.barrier.wait()
                    cnt ^= 1
            global_loss_threshold = loss_global.mean()
                    ...
                    with torch.no_grad():
                        ...
                        self.weights[cnt] = torch.cat(
                            (torch.ones(num_mis_classified, dtype=torch.float32, device=self.device),
                             torch.full((num_select_well,), 1 / self.r, device=self.device)))
                        self.inputs[cnt] = torch.cat(
                            (self.inputs[cnt][mis_classified], self.inputs[cnt][well_classified][:num_select_well]),
                            dim=0)
                        self.targets[cnt] = torch.cat(
                            (self.targets[cnt][mis_classified], self.targets[cnt][well_classified][:num_select_well]),
                            dim=0)
                    ...
                    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=True):
                        with torch.no_grad():
                            ...
                            self.inputs[cnt] = torch.cat(
                                ...
                                dim=0)
                            self.targets[cnt] = torch.cat(
                                (self.targets[cnt][mis_classified],
                                 self.targets[cnt][well_classified][:num_select_well]), dim=0)
