...
class Trainer:
    def __init__(self, args):
        ...
        self.F = [0 for i in range(self.args['client_num'])]
    ...
    def collect_client_exp(self, globelepoch_trainingtime_loss):
        D,L =[],[]

        for client_id, client_training_time, client_loss in globelepoch_trainingtime_loss: # 这时的current_selected_client_ids还等于上一轮的
            D.append(client_training_time)
            L.append(client_loss)
            self.l_min , self.l_max = min(self.l_min,client_loss) , max(self.l_max , client_loss)
            self.d_min , self.d_max = min(self.d_min,client_training_time) , max(self.d_max , client_training_time)
            cm = (client_loss - self.l_min) / (self.l_max - self.l_min) if self.l_max != self.l_min else 0.0
            ci = (self.d_max - client_training_time) / (self.d_max - self.d_min) if self.d_max != self.d_min else 0.0
            client_experience = self.alpha * ci + self.beta * cm
            self.F[client_id] = client_experience
        return max(D) 

    def get_round_diff(self,globelepoch_trainingtime_loss,loss):
        round_train_time = self.collect_client_exp(globelepoch_trainingtime_loss)
    ...
    def Client_Scheduling(self,epoch):
        ...
        if epoch >= int(self.args['global_epoch'] * 0.1):
            ...
            rho = random.choice(x)
        self.P.append(rho)
        console.log(f"the value of rho is {rho}")
        ...
        E_clients = ceil(self.args['client_num'] * self.args['client_join_ratio'] * rho)
        len_nc = int(self.args['client_num'] * self.args['client_join_ratio']) - E_clients
    ...
    def run(self):
        ...
        if option == "register":  # 一个注册进程
            ...
            with self.server_lock:
                ...
                self.server.globelepoch_trainingtime_loss[self.server.current_epoch].append((client_2_server_data['client_id'],client_2_server_data['s2c_training_time'] + client_2_server_data["client_2_server_time"], client_2_server_data['loss'])) # 记录训练时间
...
class Server:
    def __init__(self,args, socket_manager):
        ...
        self.globelepoch_trainingtime_loss = {global_epoch:[] for global_epoch in range(1 , 1 + self.total_epoches)} # {global epoch : (client_id , 整轮需要的训练时间,loss的均值)}
...
def trainingstage():
    ...
    for global_epoch in range(1,server.total_epoches + 1):
        ...
        server.trainer.Client_Scheduling(global_epoch) # 设置本轮被选中的客户
        server.trainer.prev_l = global_loss
        server.trainer.prev_t = round_train_time
        ...
        round_train_time = server.trainer.get_round_diff(server.globelepoch_trainingtime_loss[server.current_epoch],loss)

        clientId_time_list = server.globelepoch_trainingtime_loss[global_epoch]
        client_time = []
        for clientId_time_loss in clientId_time_list:
            assert clientId_time_loss[0] in server.trainer.current_selected_client_ids
            client_time.append(clientId_time_loss[1])
