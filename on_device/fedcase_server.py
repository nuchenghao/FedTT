...
class Trainer:
    ...
    def collect_client_exp(self, globelepoch_trainingtime_loss):
        D,L =[],[]
        for client_id, client_training_time, client_loss in globelepoch_trainingtime_loss:
    ...
    def run(self):
        ...
        if option == "register":
            ...
            with self.server_lock:
                ...
                self.server.globelepoch_trainingtime_loss[self.server.current_epoch].append((client_2_server_data['client_id'],client_2_server_data['s2c_training_time'] + client_2_server_data["client_2_server_time"], client_2_server_data['loss']))
...
class Server:
    def __init__(self,args, socket_manager):
        ...
        self.globelepoch_trainingtime_loss = {global_epoch:[] for global_epoch in range(1 , 1 + self.total_epoches)}
        if self.args['wandb']:
...
def trainingstage():
    ...
    for global_epoch in range(1,server.total_epoches + 1):
        ...
        server.trainer.Client_Scheduling(global_epoch)
