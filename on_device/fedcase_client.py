...
class BaseClient:
    def __init__(self, client_id, train_index, batch_size, R_S):
        ...
        self.R_S = int(self.train_set_len * R_S)
...
class Trainer:
    ...
    def client_data_sampling(self):
        experience:torch.tensor = self.trainloader.dataset.get_value(self.current_client_instance.train_set_index)
        index_sorted_experience = torch.argsort(experience)
        M_S_index = index_sorted_experience[-self.current_client_instance.num_cached:]
        r = max(math.ceil(self.alpha * (self.current_client_instance.R_S / len(M_S_index))),1)
        rep_samples = M_S_index.repeat(r)
        len_flash = self.current_client_instance.R_S - len(rep_samples)
        flash_index = index_sorted_experience[-(self.current_client_instance.num_cached+len_flash):-self.current_client_instance.num_cached]
        self.current_client_instance.selected_samples_index = self.current_client_instance.train_set_index[torch.cat((rep_samples,flash_index),dim=0).numpy()]
        self.current_client_instance.train_set_len = len(self.current_client_instance.selected_samples_index) # 更新，后续用于聚合时的参数

    def load_dataset(self):
        if self.current_client_instance.participation_times > 2:
            self.client_data_sampling()
            self.trainloader.sampler.set_index(self.current_client_instance.selected_samples_index)  # 在里面实现了深拷贝
        else:
    ...
    def start(self,global_epoch, client_instance, model_parameters):
        ...
        # 返回训练后的模型参数，训练时间
        return current_client_instance_model_dict, self.current_client_instance.training_time_record[global_epoch] , self.trainloader.dataset.get_value(self.current_client_instance.train_set_index).mean()
...
def run():
    ...
    for client_id in client.client_ids:
        client.client_instances_dict[client_id] = BaseClient(client_id, client.clientId_dataIndex[client_id], client.args["batch_size"], client.received_data['R_S'])
    ...
    while True:
        ...
        for client_id in client.current_selected_client_ids:
            ...
            current_client_instance_model_dict, training_time, loss = client.trainer.start(client.received_data['global_epoch'],client.client_instances_dict[client_id],client.model)
