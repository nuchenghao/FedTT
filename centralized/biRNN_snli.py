...
if __name__=='__main__':
    ...
    trainloader = DataLoader(trainset,
                         ...
                         collate_fn = DATASETS_COLLATE_FN[args['dataset']],prefetch_factor=4)
    ...
    testloader = DataLoader(testset, batch_size=2048, shuffle=False, pin_memory=True, num_workers=8,
                        persistent_workers=True, pin_memory_device='cuda:0',collate_fn = DATASETS_COLLATE_FN[args['dataset']],prefetch_factor=4)
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in enumerate(testloader):
                if isinstance(inputs,torch.Tensor):
                    ...
                    inputs = [tensor.to("cuda:0", non_blocking=True) for tensor in inputs]
